...
Version 3.6.2 recognizes LinkedIn's **Review** button and checks visible text, input values, accessibility labels, and titles independently. Submission labels always keep the button at manual final review. After updating, reload the extension and refresh LinkedIn before starting a fresh run.

Version 3.6.3 recognizes LinkedIn's **Applied … / See application** confirmation in the selected job's detail pane. A confirmed submission marks the tracked job Applied and lets the run continue. The final-review instruction clears when its Submit control disappears or the page changes; closing a form alone never counts as submission. Reload the extension and refresh LinkedIn to load the fix. Previously stopped runs are not restarted or retroactively marked Applied.

Version 3.6.4 makes **Start Smart Auto-Fill** match the selected LinkedIn posting, import it if necessary, generate its tailored résumé, verify the artifact, and only then upload it. Keep the popup open while generation completes. A ready résumé for that exact job is reused; failed/missing artifacts block upload. Switching jobs stops the upload, and old tab-level upload notices are ignored. Reload the extension and refresh LinkedIn before testing; final submission remains manual.

Version 3.6.7 gives automated uploads a job and content version in the filename, such as `FirstLast-Resume-J124-a1b2c3d4e5f6.docx`, so an older saved file cannot pass as the new resume. The worker scrolls the current Upload control into view, uploads through its native chooser, and requires the matching file to be selected. The dashboard upload watcher probes LinkedIn’s accessibility tree every two seconds even when the content script cannot see the application dialog or a file input; it keeps watching through contact steps and stops after a verified upload, a failure, or ten minutes. The Auto Apply final-review check compares the attached version against the current generated resume and stops for manual review if it cannot verify a match. Reload the unpacked extension and start a fresh run to activate this update. Normal manual downloads retain their existing filenames.
